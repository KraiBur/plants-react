...
function Drawer({ onClose, onRemove, items = [], opened }) {
    ...
    const onClickOrder = async () => {
        try {
            setIsLoading(true);
            const { data } = await axios.post('https://7718ff162f32ee05.mokky.dev/orders', {
                items: cartItems,
            });
            setOrderId(data.id);
            ...
            for (let i = 0; i < cartItems.length; i++) {
                ...
                await axios.delete('https://7718ff162f32ee05.mokky.dev/cart/' + item.id);
                await delay(1000);
            }
        } catch (error) {
      ...
      <div className={`${drawerStyles.overlay} ${opened ? drawerStyles.overlayVisible : ''}`}>
        ...
        <div className={drawerStyles.drawer}>
            ...
            <Info
               ...
               description={
                   isOrderComplete
                      ? `Ваш заказ #${orderId} скоро будет отправлена ссылка, ожидайте~`
